...
test_population() ->
    ...
    % {ok,Z2} = zombie_fsm:start_link(5,6,test_tile,V),
    % {ok,Z3} = zombie_fsm:start_link(7,7,test_tile,V),
    tile:summon_entity(test_tile,{Z1,{1,2}}),
    % tile:summon_entity(test_tile,{Z2,{5,6}}),
    % tile:summon_entity(test_tile,{Z3,{7,7}}),
    ?assertEqual(tile:get_population(test_tile), [[0,1,2]]).
