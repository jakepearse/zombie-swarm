...
findline(X1,X2,Y1,Y2,M,C,Olist) when X1 < X2 ->
  ...
  case Y1<Y2 of
  true ->
    %Xs = [{CheckX,CheckY} || CheckX <- lists:seq(X1,X2), CheckY <- [round(M*CheckX+C)]],
    Ys = [{CheckX,CheckY} || CheckY <- lists:seq(Y1,Y2), CheckX <- [(CheckY-C) / M]];
    %error_logger:error_report({"Xs",Xs,"Ys",Ys});
  ...
  %error_logger:error_report({"F1",F}),
  %error_logger:error_report({"Xs",Xs,"Ys",Ys}),
  filterlist(F,Olist);
...
findline(X1,X2,Y1,Y2,M,C,Olist) when X1 > X2 ->
  ...
  F=sets:to_list(sets:from_list(Xs++Ys)),
  %error_logger:error_report({"F2",F}),
   %error_logger:error_report({"Xs",Xs,"Ys",Ys}),
  filterlist(F,Olist).
